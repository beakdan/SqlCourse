IF NOT EXISTS (
	...
	WHERE	name = 'OrdersTest'
)
BEGIN
	CREATE XML SCHEMA COLLECTION dbo.OrdersTest AS N'
...
DECLARE @XmlData AS XML(dbo.OrdersTest) = N'<Orders>
  <Order>
  ...
  <Order>
    ...
    <Person FirstName="Takiko" MiddleName="J." LastName="Collins">
		...
		<Email Address="takiko0@hotmail.com" ModifiedDate="2011-05-31T00:00:00" />
		<Email Address="takiko0@gmail.com" ModifiedDate="2011-05-31T00:00:00" />
    </Person>
  </Order>
  <Order>
    <SalesOrderID>2147483648</SalesOrderID>
    <OrderDate>2011-05-31T00:00:00</OrderDate>
...
SELECT	ord.x.value('SalesOrderID', 'INTEGER') AS SalesOrderID,
		ord.x.value('OrderDate', 'DATETIME') AS OrderDate,
		ord.x.value('Money/@SubTotal', 'MONEY') AS SubTotal,
		ord.x.value('Money/@TaxAmt', 'MONEY') AS TaxAmt,
		ord.x.value('Money/@Freight', 'MONEY') AS Freight,
		ord.x.value('Money/@TotalDue', 'MONEY') AS TotalDue,
		ord.x.value('Person/@FirstName', 'NVARCHAR(50)') AS FirstName,
		ord.x.value('Person/@MiddleName', 'NVARCHAR(50)') AS MiddleName,
		ord.x.value('Person/@LastName', 'NVARCHAR(50)') AS LastName,
		eml.eml.value('./@Address', 'NVARCHAR(50)') AS EmailAddress,
		eml.eml.value('./@ModifiedDate', 'DATETIME') AS LastName
FROM	@XmlData.nodes('./Orders/Order') ord(x)
		OUTER APPLY ord.x.nodes('Person/Email') eml(eml);
