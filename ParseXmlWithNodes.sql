...
SELECT	ord.x.value('SalesOrderID[1]', 'INTEGER') AS SalesOrderID,
		ord.x.value('OrderDate[1]', 'DATETIME') AS OrderDate,
		ord.x.value('Money[1]/@SubTotal', 'MONEY') AS SubTotal,
		ord.x.value('Money[1]/@TaxAmt', 'MONEY') AS TaxAmt,
		ord.x.value('Money[1]/@Freight', 'MONEY') AS Freight,
